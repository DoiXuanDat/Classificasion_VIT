...
class Encoder(nn.Module):
    def __init__(self, num_hidden_layers: int, hidden_size: int, intermediate_size: int, num_attention_heads: int = 4,
        ...
        for _ in range(num_hidden_layers):
            block = EncoderBlock(hidden_size, intermediate_size, num_attention_heads, attention_probs_dropout_prob,
                                 hidden_dropout_prob, qkv_bias).to(device)
...
class EncoderBlock(nn.Module):
    def __init__(self, hidden_size: int, intermediate_size: int, num_attention_heads: int = 4,
                 attention_probs_dropout_prob: float = 0.0, hidden_dropout_prob: float = 0.0, qkv_bias=True):
...
class MultiHeadAttention(nn.Module):
    def __init__(self, hidden_size: int, num_attention_heads: int = 4, attention_probs_dropout_prob: float = 0.0,
                 hidden_dropout_prob: float = 0.0, qkv_bias: bool = True):
        ...
        for _ in range(self.num_attention_heads):
            head = Scaled_Dot_Product_Attention(self.hidden_size, self.attention_head_size,
                                                attention_probs_dropout_prob, bias=self.qkv_bias).to(device)
...
class Embeddings(nn.Module):
    def __init__(self, hidden_size: int, image_size: int, patch_size: int = 16, num_channels: int = 3,
    ...
    def _init_weights(self, module, initializer_range: float = 0.02):
        if isinstance(module, (nn.Linear, nn.Conv2d)):
            ...
            module.position_embeddings.data = nn.init.trunc_normal_(module.position_embeddings.data.to(torch.float32),
                                                                    mean=0.0,
                                                                    std=initializer_range).to(
                module.position_embeddings.dtype)
